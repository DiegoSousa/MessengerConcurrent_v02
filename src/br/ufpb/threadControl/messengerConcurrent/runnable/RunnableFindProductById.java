...
public class RunnableFindProductById implements Runnable {
	private IProductManager productCrud;
	private Long id;
	private BlockingQueue<Product> list;

	public RunnableFindProductById(Long id, BlockingQueue<Product> list, IProductManager iProductManager) {
	...
	public void run() {
		try {
			list.put(productCrud.findProductById(id));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
